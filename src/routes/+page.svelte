<script lang="ts">
	...
	const mentionedPubkeys: Record<string, string> = {};
	...
	class ThreadPool {
		...
		async exec<T>(cb: () => Promise<T>) {
			return new Promise<T>((ok) => {
				this.queue.push(async () => {
					ok(await cb());
				});
				console.log('ThreadPool queue', this.queue.length, 'threads', this.threads);
				if (this.threads < this.maxThreads) this.startTask();
			});
		}

		private async startTask() {
			const task = this.queue.shift();
			console.log('ThreadPool start queue', this.queue.length, 'threads', this.threads);
			this.threads++;
			await task!();
			this.threads--;
			console.log('ThreadPool done queue', this.queue.length, 'threads', this.threads);
			if (this.queue.length > 0 && this.threads < this.maxThreads) this.startTask();
		}
	}

	const apiThreadPool = new ThreadPool();
	const fetchMentionedPubkey = async (screenName: string) => {
		//        return apiThreadPool.exec(async () => {
		return await (async () => {
			console.log('fetchMentionedPubkey', screenName);
			if (screenName in mentionedPubkeys) return mentionedPubkeys[screenName];
			...
			try {
				...
				// // FIXME verify
				// if (profiles.size) {
				// 	pubkey = [...profiles.values()][0].pubkey;
				// }

				if (!pubkey) {
					// read from nostrdirectory through nostr.band
					try {
						const r = await fetch(`https://api.nostr.band/v0/twitter_pubkey/${screenName}`);
						const body = await r.json();
						if (body.twitter_handle === screenName && body.pubkey) pubkey = body.pubkey;
					} catch {}
				}

				// store in cache
				mentionedPubkeys[screenName] = pubkey;
			} catch (e) {
				console.log('fetchMentionedProfile error', e);
			}

			// don't go crazy polling the apis
	...
	const fetchTweetEvent = async (screenName: string, id: string) => {
	...
	async function createEvent(id: string): Promise<NDKEvent | undefined> {
		...
		if (replyToId) {
			...
			// if it's not, create it - only if replying to ourself
			...
			if (replyEvent) {
				...
				// use RE url as easy-to-parse marker, smart clients can cut it and
				// display an embed or fetch the replied-to event while rendering
				event.content = `RE ${replyUrl}:\n\n` + event.content;
			}
		}
		...
		if (tweet.entities.user_mentions.length > 0) {
			// const parts = [{ c: event.content, i: 0, e: event.content.length }];
			// const replace = (i: number, e: number, c: string) => {
			// 	// find part by i
			// 	const partIndex = parts.findIndex((p) => p.i <= i && p.e >= e);
			// 	if (partIndex < 0) return; // bad indices
			// 	const part = parts[partIndex];
			// 	const prefix = { i: part.i, e: i, c: part.c.substring(0, i - part.i) };
			// 	const body = { i, e, c };
			// 	const suffix = { i: e, e: part.e, c: part.c.substring(e - part.i) };
	...
	async function fetchAndUploadToSatelliteCDN(url: string): Promise<string | undefined> {
		...
		const uploadResponse = await fetch(
			`https://api.satellite.earth/v1/media/item?auth=${encodeURIComponent(
				JSON.stringify(uploadEvent.rawEvent())
			)}`,
			{
				method: 'PUT',
				body: file
			...
			await new Promise<void>((resolve) => {
				setTimeout(async () => {
					if (sequentialFailures > 10) {
						if (!continueButton) {
							alert(
								`Too many failures, stopping. Wait a few minutes and hit the "CONTINUE" button`
							);
						}
						continueButton = true;
						resolve();
...
<div class="max-w-prose mx-auto">
	<div class="flex flex-row gap-4 my-4">
		<img
			src="https://api.tasktiger.io/media/91cfbb51-f100-48ef-8945-ad5937fa848e.png"
		...
		<div class="flex flex-col gap-2 items-start">
			<h1 class="flex flex-row mx-auto items-center text-7xl font-black w-full">
				E
				<svg viewBox="0 0 24 24" aria-hidden="true" class="w-16"
					><g
						><path
	...
	{#if !nip07Available}
		<div class="alert alert-neutral my-6">
			<svg
				...
				fill="none"
				viewBox="0 0 24 24"
				class="self-start stroke-neutral shrink-0 w-6 h-6"
				><path
			...
			<div class="flex flex-col gap-4">
				<span>
					Looks like you don't have a Nostr Extension (NIP-07), you'll need to enter your nsec to
					sign events, or use EXIT from a browser with NIP-07.
				</span>
				...
				<div class="join join-horizontal">
					...
					/>
					<button class="btn btn-primary" on:click={setPrivateKeySigner}> Set </button>
				</div>
			</div>
		</div>
	{/if}
	...
	<ul class="steps steps-vertical">
		<li class="step step-primary">
			<div>
				Gently request a backup of your X data <a
		...
		<li class="step" class:step-primary={!!myScreenName}>
			<div>
	...
	<div class="py-4" class:hidden={satsPerTweet === undefined}>
		<h1 class="text-xl">
			...
			Enter an amount of sats you'd like to ⚡️ zap me for each imported tweet
		</h1>
	...
	{#if tweetCount > 0 && processedTweetCount === tweetCount}
		<div class="form-control">
			<label class="label cursor-pointer w-fit">
				...
				<span class="label-text w-fit"> Include threads </span>
			</label>
		</div>
		...
		<div class="form-control">
			<label class="label cursor-pointer w-fit">
				...
				<span class="label-text w-fit"> Include Replies </span>
			</label>
		</div>
		...
		<div class="form-control">
			<label class="label cursor-pointer w-fit">
				...
				<span class="label-text w-fit"> Include Retweets </span>
			</label>
		</div>
	{/if}
	...
	<button
		...
		on:click={createEvents}
		disabled={tweetsToImport.length === 0 || !pubkey}
	>
		Preview events
		{#if generatingEvents}
			<span class="loading loading-lg ml-2" />
		{/if}
	</button>
		...
		<div class="mockup-browser border bg-base-300 my-2">
			...
			<div class="px-4 py-4 bg-base-200 max-h-[50vh] overflow-y-auto overflow-x-hidden">
				{#each Array.from(events.values()) as event}
					{#if (!filter || event.content
							.toLowerCase()
							.includes(filter.toLowerCase())) && !publishedEvents.has(event.id)}
						<label
							...
							<div class="flex flex-col gap-2 w-full">
								...
								<div class="text-xs">
									...
									{#if publishedEvents.has(event.id)}
										<a
											href="https://njump.me/{event.encode()}"
											target="_blank"
											class="text-success inline-block w-64"
										>
											<span class="truncate">
												{event.encode()}
											</span>
							...
							<div class="flex flex-col gap-2 w-full">
								...
								<div class="text-xs flex flex-row items-center justify-between">
									...
									<a
										href="https://njump.me/{event.encode()}"
										target="_blank"
										class="text-success inline-block w-64 truncate"
									>
										<span class="">
											{event.encode()}
										</span>
	...
	{#if publishedEvents.size === 0}
		<div class="form-control">
			<label class="label cursor-pointer w-fit">
				...
				<span class="label-text w-fit">
					Upload images to Satellite's CDN [<a
						href="https://satellite.earth/cdn"
						class="text-primary">purchase credit here</a
					>]
				</span>
			</label>
		</div>
	...
	{#if continueButton && publishedEvents.size === 0}
		<div class="alert alert-neutral">
			<svg
				...
				fill="none"
				viewBox="0 0 24 24"
				class="stroke-info self-start shrink-0 w-6 h-6"
				><path
					stroke-linecap="round"
					stroke-linejoin="round"
					stroke-width="2"
			...
			<div class="flex flex-col gap-4">
				<p>
					Looks like your selected relays are not accepting your events; many relays do this. You
					can spin up your own relay and configure it to accept your tweets; you can also use my
					relay I created for this specific usecase.
				</p>
	...
	<div class="flex flex-row items-center gap-4">
		<button
			class="btn btn-lg btn-wide btn-primary whitespace-nowrap flex-nowrap"
			on:click={publishEvents}
			disabled={(includedEventCount === 0 || publishedEvents.size > 0) && !continueButton}
		>
			{#if !continueButton}
				Publish events
			{:else}
			...
			{#if publishedEvents.size === 0}
				{#if satsPerTweet !== undefined && satsPerTweet > 0 && includedEventCount > 0}
					<span class="text-sm font-thin"
						>{nicelyFormattedSatNumber(satsPerTweet * includedEventCount)} sats</span
					>
				{/if}
			{/if}
		</button>
	...
	{#if relayList}
		<div class="collapse bg-base-200 my-4">
			...
			<div class="collapse-content flex flex-col join join-vertical">
				<textarea
					bind:value={relayListString}
					class="join-item resize-none textarea textarea-bordered h-64"
				/>
	...
	{#if publishedEvents.size > 0 && !webln && satsPerTweet && satsPerTweet > 0}
		<div class="alert alert-info my-6">
			...
			>
			<span>
				You don't have a Web LN available; you can zap {nicelyFormattedSatNumber(
					satsPerTweet * includedEventCount
				)} sats to @pablof7z if you'd like.
			</span>
		</div>
	{/if}
</div>
